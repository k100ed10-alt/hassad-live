...
function renderDashboard(){const user=currentUser();if(!user||user.role!=="student")return;const wrap=document.getElementById("subject-grid");if(!wrap)return;}
...
const tb=document.getElementById("students"); if(tb) tb.innerHTML=Object.entries(data.students).map(([uid,st])=>`<tr><td>${st.name}</td><td>${st.email}</td><td><code>${st.password}</code></td><td>${st.grade}</td><td>${st.subscription_status}</td></tr>`).join("");
}
